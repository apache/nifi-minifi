    ...
    public static DockerComposeRule docker = DockerComposeRule.builder()
            ...
            .waitingForServices(Arrays.asList("squid", "mocknifi"),
                    new HttpsStatusCodeHealthCheck(container -> "https://mocknifi:8443/", containers -> containers.get(0), containers -> containers.get(1), () -> {
                        ...
                        try {
                            String keyStorePath = c2.resolve("keystore.jks").toFile().getAbsolutePath();
                            TlsConfiguration tlsConfiguration = new StandardTlsConfiguration(keyStorePath,
                                    "badKeystorePass",
                                    KeystoreType.JKS,
                                    c2.resolve("truststore.jks").toFile().getAbsolutePath(),
                                    "badTrustPass",
                                    KeystoreType.JKS);
                            return SslContextFactory.createSslContext( tlsConfiguration, ClientAuth.NONE).getSocketFactory();
